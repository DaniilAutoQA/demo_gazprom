...
public class GazpromBankTests extends TestBase {
    ...
    void verifyDeposits() {
        ...
        step("Проверить рассчет", () -> {
            step("Сумма в конце срока должна быть  9 528 480", () ->{
                $(".js-nn-calc-rSum").shouldHave(text("9 528 480"));
            });
            step("Доход должен быть 528 480", () ->{
                $(".js-nn-calc-rProfit").shouldHave(text("528 480"));
            });
            step("Процентная ставка должна быть 5.84%", () ->{
                $(".js-nn-calc-rRate").shouldHave(text("5.84"));
            });
